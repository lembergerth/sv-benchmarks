...
void aws_array_list_copy_harness() {
   ...
   ((void *)0)
    ...
    if (!aws_array_list_copy(&from, &to)) {
        ...
        if (!(to.length == from.length)) __VERIFIER_error();
        if (!(to.current_size >= (from.length * from.item_size))) __VERIFIER_error();
    }


    if (!(aws_array_list_is_valid(&from))) __VERIFIER_error();
    if (!(aws_array_list_is_valid(&to))) __VERIFIER_error();
    if (!(from.item_size == to.item_size)) __VERIFIER_error();
}
