   ...
   _Bool 
    ...
    if (rval == (0)) {
        if (track_was_present) {
            if (!(map.p_impl->entry_count == old_state.entry_count - was_present)) __VERIFIER_error();
        } else {
        ...
        if (get_p_elem && track_was_present && was_present == 1) {
            if (!(uninterpreted_equals(p_elem.key, key))) __VERIFIER_error();
        }
    } else {
        if (!(map.p_impl->entry_count == old_state.entry_count)) __VERIFIER_error();
    }
}
