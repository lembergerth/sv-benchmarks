...
void aws_linked_list_back_harness() {
    ...
    if (!(aws_linked_list_is_valid(&list))) __VERIFIER_error();
    if (!(aws_linked_list_node_prev_is_valid(back))) __VERIFIER_error();
    if (!(aws_linked_list_node_next_is_valid(back))) __VERIFIER_error();
    if (!(back == old_last)) __VERIFIER_error();
}
