...
void aws_linked_list_remove_harness() {
    ...
    if (!(aws_linked_list_node_next_is_valid(&prev))) __VERIFIER_error();
    if (!(aws_linked_list_node_prev_is_valid(&next))) __VERIFIER_error();

    if (!(prev.next == &next)) __VERIFIER_error();
    __VERIFIER_assert(node.next == 
   ((void *)0)
   );
