...
void aws_hash_iter_next_harness() {
    ...
    if (!(aws_hash_iter_is_valid(&iter))) __VERIFIER_error();
    if (!(iter.status == AWS_HASH_ITER_STATUS_DONE || iter.status == AWS_HASH_ITER_STATUS_READY_FOR_USE)) __VERIFIER_error();
    if (!((!(old_status == AWS_HASH_ITER_STATUS_DONE) || (iter.status == AWS_HASH_ITER_STATUS_DONE)))) __VERIFIER_error();
    if (!(aws_hash_table_is_valid(&map))) __VERIFIER_error();
