...
void aws_hash_iter_delete_harness() {
    ...
    __VERIFIER_assume(aws_hash_table_is_valid(&map));
    __VERIFIER_assume(map.p_impl->destroy_key_fn == hash_proof_destroy_noop || !map.p_impl->destroy_key_fn);
    __VERIFIER_assume(map.p_impl->destroy_value_fn == hash_proof_destroy_noop || !map.p_impl->destroy_value_fn);
    ...
    if (!(iter.status == AWS_HASH_ITER_STATUS_DELETE_CALLED)) __VERIFIER_error();
    if (!(aws_hash_iter_is_valid(&iter))) __VERIFIER_error();
}
