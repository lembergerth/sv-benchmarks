...
void aws_linked_list_front_harness() {
    ...
    if (!(aws_linked_list_is_valid(&list))) __VERIFIER_error();
    if (!(aws_linked_list_node_prev_is_valid(front))) __VERIFIER_error();
    if (!(aws_linked_list_node_next_is_valid(front))) __VERIFIER_error();
    if (!(front == old_first)) __VERIFIER_error();
}
