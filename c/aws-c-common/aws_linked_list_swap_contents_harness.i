...
int aws_array_list_front(const struct aws_array_list *restrict list, void *val) {
    ...
    if (aws_array_list_length(list) > 0) {
        ...
        if (!(((memcmp(val, list->data, list->item_size) == 0)))) __VERIFIER_error();
        if (!((aws_array_list_is_valid(list)))) __VERIFIER_error();
        return (0);
    }
...
void aws_linked_list_swap_contents_harness() {
    ...
    if (aws_linked_list_empty(&a)) {
        if (!(old_b.tail.prev == old_b_head)) __VERIFIER_error();
    } else {
        if (!(a.head.next == old_b.head.next)) __VERIFIER_error();
        if (!(a.tail.prev == old_b.tail.prev)) __VERIFIER_error();
    }

    if (aws_linked_list_empty(&b)) {
        if (!(old_a.tail.prev == old_a_head)) __VERIFIER_error();
    } else {
        if (!(b.head.next == old_a.head.next)) __VERIFIER_error();
        if (!(b.tail.prev == old_a.tail.prev)) __VERIFIER_error();
    }
}
