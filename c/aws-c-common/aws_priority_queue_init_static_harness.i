...
void aws_priority_queue_init_static_harness() {
    ...
    __VERIFIER_assume(initial_item_allocation > 0 && initial_item_allocation <= 9223372036854775808U);
   ...
   ((void *)0)
    ...
    if (!(queue.container.item_size == item_size)) __VERIFIER_error();
    if (!(queue.container.length == 0)) __VERIFIER_error();
    if (!(queue.container.current_size == initial_item_allocation * item_size)) __VERIFIER_error();
